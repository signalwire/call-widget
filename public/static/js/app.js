...
class DemoCall {
  ...
  _initializeEventListeners() {
    ...
    document.addEventListener('click', (event) => {
      if (this.elements.deviceSelectors?.classList.contains('show') &&
    ...
    try {
      ...
      if (this.localStream) {
        const oldTrack = changedDeviceType === 'audioInput' ? 
          this.localStream.getAudioTracks()[0] : this.localStream.getVideoTracks()[0];
  ...
  _updateUI(isShowing) {
    ...
    this.elements.placeholder.style.display = isShowing && !isCallActive ? 'flex' : 'none';
